...
local function loadState()
    ...
    if not raw or raw == '' then
        print('[av_ipls] state.json empty or missing -- starting fresh')
        return {}
    end
    local ok, parsed = pcall(json.decode, raw)
    if ok and type(parsed) == 'table' then return parsed end
    print('[av_ipls] WARN: state.json malformed -- starting fresh')
    return {}
end
...
local function saveState()
    ...
    if not ok then
        print('[av_ipls] ERROR: could not encode state')
        return
    end
    local written = SaveResourceFile(GetCurrentResourceName(), STATE_FILE, encoded, -1)
    if not written then
        print('[av_ipls] ERROR: SaveResourceFile failed for ' .. STATE_FILE)
    end
end

-- Seed state.json with interiors missing from the registry
...
local function seedMissingInteriors()
    ...
    if not ok or type(Registry) ~= 'table' then
        print('[av_ipls] WARN: registry load failed for seed -- ' .. tostring(Registry))
        return
    end
    ...
    for id, def in pairs(Registry) do
        if not activeState[id] then
            ...
            seeded = seeded + 1
            print('[av_ipls] seed: ' .. id)
        end
    end

    if seeded > 0 then
        saveState()
        print('[av_ipls] seeded ' .. seeded .. ' new interiors into state.json')
    end
end
...
AddEventHandler('onResourceStart', function(res)
    ...
    print('[av_ipls] ready -- ' .. total .. ' entries, ' .. active .. ' active')
end)
...
RegisterNetEvent('av_ipls:batchUpdate', function(payload)
    local src = source
    if not isAdmin(src) then
        print('[av_ipls] batchUpdate: denied for src=' .. tostring(src))
        return
    end
    if type(payload) ~= 'table' then
        print('[av_ipls] batchUpdate: invalid payload type=' .. type(payload))
        return
    end
    local changed = {}
        ...
        if intId and type(intId) == 'string' then
            ...
            if type(entry.variants) == 'table' then
                for group, option in pairs(entry.variants) do
                    activeState[intId][group] = option
                    print('[av_ipls] batchUpdate: ' .. intId .. '.' .. group .. ' = ' .. tostring(option))
                end
            end
            changed[#changed + 1] = intId
            print('[av_ipls] batchUpdate: saved ' .. intId .. ' active=' .. tostring(entry.active))
        end
    end
    saveState()
    for _, intId in ipairs(changed) do
        print('[av_ipls] batchUpdate: broadcasting ' .. intId .. ' to all clients')
        broadcastInterior(intId, activeState[intId])
    end
    print('[av_ipls] batchUpdate: done, ' .. #changed .. ' interiors broadcast')
end)
